...
methods.set('/posts.post', async ({response,searchParams, db}) => {
  ...
  const resultSelect = await table.select(['id', 'content', 'likes', 'created'])
    .where('id = :id && removed = FALSE')
    .orderBy('id DESC')
    .bind('id', id)
    .execute();
  
  const data = resultSelect.fetchAll();
  const columns = resultSelect.getColumns();
  const posts = data.map(map(columns));
...
methods.set('/posts.edit', async ({response,searchParams, db}) => {
  ...
  const content = searchParams.get('content');
  const table = await db.getTable('posts');
  const result = await table.update().set('content', content).where('id = :id and removed = false')
  .bind('id', id)
  .execute();
  const count = result.getAffectedItemsCount();
  if (count <= 0){
    sendResponse(response, {status: statusNotFound});
    return;
  }
...
methods.set('/posts.like', async ({response,searchParams, db}) => {
  ...
  const result = await table.update().set('likes', Number(posts[0].likes) + 1).where('id = :id and removed = false')
  .bind('id', id)
  .execute();
  const count = result.getAffectedItemsCount();
  if (count <= 0){
  ...
  console.log(data);
  posts[0].likes = Number(posts[0].likes) + 1;
  sendJSON(response, posts[0]);
});
